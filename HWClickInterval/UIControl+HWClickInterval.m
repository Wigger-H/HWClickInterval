...
#import "UIControl+HWClickInterval.h"
...
// 上次按钮响应的方法名
@property (nonatomic, copy) NSString *oldSELName;
@end
@implementation UIControl (HWClickInterval)
+(void)hw_exchangeClickMethod{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        //获得方法选择器
        SEL originalSel = @selector(sendAction:to:forEvent:);
        SEL newSel = @selector(hw_sendClickIntervalAction:to:forEvent:);
        //获得方法
        Method originalMethod = class_getInstanceMethod(self, originalSel);
        Method newMthod = class_getInstanceMethod(self, newSel);
        
        //如果发现方法已经存在，返回NO；也可以用来做检查用,这里是为了避免源方法没有存在的情况;如果方法没有存在,我们则先尝试添加被替换的方法的实现
        BOOL isAddNewMethod = class_addMethod(self, originalSel, method_getImplementation(newMthod), "v@:");
        if (isAddNewMethod) {
...
-(void)hw_sendClickIntervalAction:(SEL)action to:(id)target forEvent:(UIEvent *)event{
    if ([self isKindOfClass:[UIButton class]] && !self.ignoreClickInterval) {
        ...
        if (self.clickInterval > 0) {
            ...
            [self performSelector:@selector(hw_ignoreClickState:) withObject:@(NO) afterDelay:self.clickInterval];
        }
    }
    [self hw_sendClickIntervalAction:action to:target forEvent:event];
}
-(void)hw_ignoreClickState:(NSNumber *)ignoreClickState{
